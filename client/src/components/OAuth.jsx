...
function OAuth() {
    ...
    const handelGoogle = async () => {
        try {
            ...
            const data = { username: result.user.displayName, email: result.user.email, imgurl: result.user.photoURL }
            await axios.post('http://localhost:3000/auth/google', data)
                .then(e => {
                    console.log('googleData:', e.data)
